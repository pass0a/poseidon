            ...
            <el-row :gutter="10">
                <el-col :span="5"> 
                    <el-card class="box-card" style="text-align:center;" shadow="never">
                        ...
                        <button class="btn_stop" v-show="btnMode==3"><i class="el-icon-loading"></i>暂停中...</button>
                        <span v-show="btnMode==4">
                        <button class="btn_continue" @click="continueTest()"><i class="el-icon-video-play"></i>继续测试</button>
                        <button class="btn_replay" @click="replayTest()"><i class="el-icon-refresh-left"></i>重新测试</button>
                        </span>
                    </el-card>
                </el-col>
                <el-col :span="19">
                    <el-card class="box-card" shadow="never">
                        ...
                        <div style="margin:18px 0px 12px 10px">
                            ...
                            <span style="margin:0px 0px 0px 10px"><font size="3" :color="statusColor.c_status">{{statusTitle.t_status}}</font></span>
                            <span style="font-size: 15px;" v-show="btnMode==2||btnMode==3"><i class="el-icon-loading" ></i></span>
                            <span style="margin:0px 0px 0px 20px"><font size="4"><strong>预计时间: </strong></font></span>
                            <span style="margin:0px 0px 0px 10px"><font size="3" :color="statusColor.c_time">{{statusTitle.t_time}}</font></span>
                            <span style="margin:0px 0px 0px 20px"><font size="4"><strong>测试进度: </strong></font></span>
                            <span style="margin:0px 0px 0px 10px"><font size="3" :color="statusColor.c_progress">{{statusTitle.t_progress}}</font></span>
                            <span style="margin:0px 0px 0px 20px"><font size="4"><strong>测试结果: </strong></font></span>
                            <span style="margin:0px 0px 0px 10px"><font size="3" :color="statusColor.c_result_ok">{{"OK: " + statusTitle.t_result_ok}}</font></span>
                            <span style="margin:0px 0px 0px 10px"><font size="3" :color="statusColor.c_result_ng">{{"NG: " + statusTitle.t_result_ng}}</font></span>
                        </div>
                    </el-card>
                </el-col>
		    </el-row>
...
export default class TestView extends Vue {
    ...
    private statusTitle:any={t_status:"未启动",t_time:"- - - -",t_result_ok:"- - -",t_result_ng:"- - -",t_progress:"- - - -"};
    private statusColor:any={c_status:"#C0C4CC",c_time:"#C0C4CC",c_result_ok:"#C0C4CC",c_result_ng:"#C0C4CC",c_progress:"#C0C4CC"};
    private logCmd:any="";
        ...
        if(this.btnMode){
            switch(this.btnMode){
                ...
                case 6:
                    ...
                    this.$notify({title: "测试已完成",message: '', type: 'success',duration:1500});
                    break;
            }
        }else{
            //Progress
    ...
    get updateTestStatus(){
        ...
        switch(this.testInfo.mode){
            case 0:
                this.btnMode=5;
                this.$notify({title: '测试异常!请查看测试输出!',message: '', type: 'error',duration:1500});
                break;
            case 1:
                this.btnMode=2;
                break;
