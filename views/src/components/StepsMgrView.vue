  ...
  <div>
    ...
    >
      ...
      <el-button
        ...
        size="small"
        @click="addID(2)"
        v-show="actionName.length>1&&actionName=='button'"
      >添加硬按键</el-button>
      <div v-show="actionName.length>1&&actionName=='dbc'">
        <el-button
          ...
          @click="openDBC()"
        >打开DBC文件</el-button>
        <span style="margin:0px 0px 10px 30px;">
          <font size="2">当前打开DBC文件 : {{ dbcPath }}</font>
        </span>
      </div>
      <el-tabs tab-position="left" v-model="moduleName" @tab-click="clickModule()">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button type="text" @click="checkImg(scope.row.id)">
                <font size="2" :color="bingId(0,scope.row.id)">{{bingId(1,scope.row.id)}}</font>
              </el-button>
            </template>
          </el-table-column>
          <el-table-column
            prop="content"
            label="绑定信息"
            v-if="actionName=='click_poi'||actionName=='slide'"
          >
            <template slot-scope="scope">
              <span>
                <font size="2" :color="bingId(0,scope.row.id)">{{ getBindInfo(scope.row.id) }}</font>
              </span>
            </template>
          </el-table-column>
    ...
    <Modal v-model="checkflag" width="600" :mask-closable="false">
      <p slot="header">
    ...
    >
      ...
      >
        ...
        <div v-if="add_info.action=='click_random'&&add_type">
    ...
    >
      ...
      >
        ...
        <div v-if="actionName=='click_random'&&edit_info.type">
          <el-form-item label="尺寸大小 :">
            <el-input-number
              v-model="edit_info.w"
              controls-position="right"
...
export default class StepsMgrView extends Vue {
  ...
  get Module() {
    ...
    if (this.moduleName == "" || this.moduleName == 0) {
      if (
        this.rulelist[this.actionName] != undefined &&
        this.rulelist[this.actionName].length
      ) {
        this.moduleName = this.rulelist[this.actionName][0];
      }
    }
    return this.rulelist[this.actionName] != undefined
      ? this.rulelist[this.actionName]
      : [];
  }
  get moduleList() {
  ...
  private editID(type: number, id: any) {
    ...
    if (type) {
      if (id.indexOf("button") > -1) {
        ...
        this.$store.state.steps_info.steplist = JSON.parse(
          JSON.stringify(this.$store.state.steps_info.grouplist[id])
        );
        ...
        this.$store.state.pcan_info.data = JSON.parse(
          JSON.stringify(this.$store.state.steps_info.pcanlist[id])
        );
      } else if (id.indexOf("click_random") > -1) {
        let info = JSON.parse(
          JSON.stringify(this.$store.state.steps_info.bindlist[id])
        );
        this.edit_info.w = info.content.w;
        this.edit_info.h = info.content.h;
      }
    }
    this.editflag = true;
  }
        ...
        switch (this.add_type) {
          ...
          case 1:
            ...
            if (this.add_info.action == "group") {
              ...
              this.add_info.grouplist = JSON.stringify(
                this.$store.state.steps_info.steplist
              );
            ...
            } else if (this.add_info.action == "click_random") {
              req.pid = this.add_info.module;
              req.content = {
                w: this.add_info.w,
                h: this.add_info.h
          ...
          case 2:
            ...
            req.content = [
              [this.add_info.event_down_1, this.add_info.event_down_2],
              [this.add_info.event_up_1, this.add_info.event_up_2]
            ];
            break;
        }
  ...
  private bingId(type: number, id: string) {
    switch (type) {
      case 0:
        return this.$store.state.steps_info.bindlist[id]
          ? "#67C23A"
          : "#F56C6C";
      case 1:
  ...
  private blur_size(type: number) {
    if (!type) {
      if (this.add_info.w == undefined) this.add_info.w = 780;
      if (this.add_info.h == undefined) this.add_info.h = 400;
    } else {
      if (this.edit_info.w == undefined) this.edit_info.w = 780;
      if (this.edit_info.h == undefined) this.edit_info.h = 400;
    }
  }
