    ...
    <el-container>
      ...
      >
        <el-menu-item index="1">
          <i class="el-icon-folder-opened"></i>打开项目
        </el-menu-item>
        <el-menu-item index="2">
          <i class="el-icon-folder-add"></i>新建项目
        </el-menu-item>
        <el-menu-item index="3">
          <i class="el-icon-edit-outline"></i>编辑用例
        </el-menu-item>
        <el-menu-item index="4">
          <i class="el-icon-monitor"></i>进入测试
        </el-menu-item>
      ...
      <div style="background-color: #545c64bb;height:31px;">
        ...
        <span style="margin:0px 0px 0px 10px">
          <font size="2" color="#ffd04b	">
            <i class="el-icon-monitor"></i> 服务器
          </font>
        </span>
        <span style="margin:0px 0px 0px 0px">
          <font size="2" :color="showConnectStatus(0,connectStatus.server)">
            (
            <i class="el-icon-loading" v-show="!connectStatus.server"></i>
            {{showConnectStatus(1,connectStatus.server)}})
          </font>
        </span>
        <span style="margin:0px 0px 0px 10px">
          <font size="2" color="#ffd04b">
            <i class="el-icon-coin"></i> 数据库
          </font>
        </span>
        <span style="margin:0px 0px 0px 0px">
          <font size="2" :color="showConnectStatus(0,connectStatus.db)">
            (
            <i class="el-icon-loading" v-show="!connectStatus.db"></i>
            {{showConnectStatus(1,connectStatus.db)}})
          </font>
        </span>
        <span style="margin:0px 0px 0px 10px">
          <font size="2" color="#ffd04b">
            <i class="el-icon-link"></i> LINK
          </font>
        </span>
        <span style="margin:0px 0px 0px 0px">
          <font size="2" :color="showConnectStatus(0,connectStatus.link)">
            (
            <i class="el-icon-loading" v-show="!connectStatus.link"></i>
            {{showConnectStatus(1,connectStatus.link)}})
          </font>
        </span>
        <span>
          <font size="4">
        ...
        <button
          ...
          v-show="connectStatus.server==2||connectStatus.db==2||connectStatus.link==2"
          @click="reconnect"
        >点击重新连接失败项</button>
      </div>
      <div>
        <ul v-show="select_mode=='3'">
          <EditCasesView />
        </ul>
        <ul v-show="select_mode=='4'">
          <TestView />
        </ul>
        <ul v-show="select_mode=='5'">
          <ReportView />
        </ul>
        <ul v-show="select_mode=='6_1'">
          <ModuleView />
        </ul>
...
export default class Home extends Vue {
  ...
  private selectMode(key: any) {
    if (
      this.connectStatus.server != 1 ||
      this.connectStatus.db != 1 ||
      this.connectStatus.link != 1
    ) {
      this.$notify({
        title: "请检查连接状态!",
        message: "",
    ...
    if (this.select_mode != key) {
      ...
      switch (key) {
        ...
        case "3":
          ...
          if (c_module.length) {
            if (
              this.$store.state.editcase_info.firstModule.length > 0 &&
              c_module.indexOf(this.$store.state.editcase_info.firstModule) > -1
            ) {
            } else {
        ...
        case "5":
          ...
          if (p_module.length) {
            this.$store.state.app_info.pis.write({
              ...
              route: "results",
              job: "info",
              info: { prjname: p_pname }
            });
            if (
              this.$store.state.test_info.first_module != "" &&
              p_module.indexOf(this.$store.state.test_info.first_module) > -1
            ) {
              this.$store.state.report_info.firstModule = this.$store.state.test_info.first_module;
            } else {
            ...
            this.$store.state.app_info.pis.write({
              ...
              info: {
                prjname: p_pname,
                module: this.$store.state.report_info.firstModule
              }
            });
          } else {
