<template>
  <div>
    ...
    <el-link v-model="uploadEnd" v-show="false"></el-link>
    <el-card class="box-card" shadow="never" style="margin:5px 10px 5px 10px;">
      <span>
        <font size="4">
          <strong>测试开始时间:</strong>
        </font>
      </span>
      <span style="margin:0px 0px 0px 10px">
        <font size="3">{{report_info.startTime}}</font>
      </span>
      <span style="margin:0px 0px 0px 20px">
        <font size="4">
          <strong>测试结束时间:</strong>
        </font>
      </span>
      <span style="margin:0px 0px 0px 10px">
        <font size="3">{{report_info.endTime}}</font>
      </span>
      <span style="margin:0px 0px 0px 20px">
        <font size="4">
          <strong>测试总耗时长:</strong>
        </font>
      </span>
      <span style="margin:0px 0px 0px 10px">
        <font size="3">{{report_info.testTime}}</font>
      </span>
      <span style="margin:0px 0px 0px 20px">
        <font size="4" color="#67C23A">
          <strong>OK:</strong>
        </font>
      </span>
      <span style="margin:0px 0px 0px 10px">
        <font size="3" color="#67C23A">{{report_info.okCount}}</font>
      </span>
      <span style="margin:0px 0px 0px 20px">
        <font size="4" color="#F56C6C">
          <strong>NG:</strong>
        </font>
      </span>
      <span style="margin:0px 0px 0px 10px">
        <font size="3" color="#F56C6C">{{report_info.ngCount}}</font>
      </span>
      <span style="margin:0px 0px 0px 20px">
        <font size="4">
          <strong>通过率:</strong>
      ...
      <span style="margin:0px 0px 0px 10px">
        <font size="3">{{report_info.passRate}}</font>
      </span>
      <el-button type="info" style="float:right" @click="openPushResults">上传测试结果</el-button>
    </el-card>
    <el-tabs
      type="border-card"
      tab-position="bottom"
          ...
          <template slot-scope="scope">
            <span>
              <strong>
                <font
                  size="2"
                  :color="!scope.row.result?'#67C23A':'#F56C6C'"
                >{{ !scope.row.result?'OK':'NG' }}</font>
              </strong>
            </span>
          </template>
        </el-table-column>
    ...
    <Modal v-model="showflag" width="320" :closable="false" :mask-closable="false">
      <p slot="header">
        <i class="el-icon-edit"></i>
        <span>修正结果</span>
      </p>
      <el-radio-group v-model="result" style="margin: 20px">
        <el-radio label="OK" border>
          <font color="#67C23A">OK</font>
        </el-radio>
        <el-radio label="NG" border>
          <font color="#F56C6C">NG</font>
        </el-radio>
      </el-radio-group>
      <span slot="footer">
    ...
    <Modal v-model="uploadflag" width="380" :closable="false" :mask-closable="false">
      <p slot="header">
        <i class="el-icon-upload"></i>
        <span>上传结果</span>
      </p>
      <el-select v-model="uploadct" placeholder="请选择上传内容" style="width:300px">
        <el-option
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="upload" v-if="!uploading">上传</el-button>
...
export default class ReportView extends Vue {
  ...
  get uploadEnd() {
    if (this.$store.state.report_info.uploadCount > 0) {
      this.uploadflag = false;
    }
    return;
  }
  private openPushResults() {
  ...
  private upload() {
    ...
    this.$store.state.app_info.pis.write({
      ...
      info: {
        ...
        pid: this.$store.state.project_info.current_pid,
        vid: this.$store.state.project_info.current_vid,
        ct: this.uploadct
      }
    });
  }
  private dblClickCell(row: any, column: any) {
    if (column.property == "result") {
      this.showflag = true;
      this.result = !row.result ? "OK" : "NG";
      this.selectRow = row;
    }
  }
  private cancel() {
  ...
  private ok() {
    let ret = !this.selectRow.result ? "OK" : "NG";
    if (this.result != ret) {
      this.$store.state.app_info.pis.write({
        type: "toDB",
        route: "results",
  ...
  private setReportInfo(data: any) {
    if (data != null) {
      ...
      this.report_info.testTime = this.disposedTestTime(
        data.startTime,
        data.endTime
      );
      ...
      this.report_info.passRate =
        ((data.ok * 100) / (data.ng + data.ok)).toFixed(2) + "%";
    }
  }
  ...
  private setReportData(data: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      case_info._id = data[i]._id;
      case_info.cid = data[i].cid;
