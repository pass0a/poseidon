    ...
    <div>
        <el-form :inline="true" label-position="left" ref="portform" :model="editInfo" label-width="100px">
            <el-form-item label="Prot">
                ...
                <p><font color="red">{{ formTit(0) }}</font></p>
            </el-form-item>
            <el-form-item label="Baud Rate" style="margin:0px 0px 0px 50px">
                <el-input-number v-model="initInfo.baud_rate" controls-position="right" :min="0" size="small"></el-input-number>
                <p><font color="red">{{ formTit(1) }}</font></p>
            </el-form-item>
...
export default class SetPort extends Vue {
    private initInfo={port:1,baud_rate:115200,data_bits:8,stop_bits:0,parity:0,flow_control:0};
    private dbits=[5,6,7,8];
	private sbits=[{name:"1",num:0},{name:"1.5",num:1},{name:"2",num:2}];
