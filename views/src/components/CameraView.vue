<template>
    <div>
        ...
        <el-card :body-style="{ padding: '8px' }" style="margin:5px 10px 0px 10px;" shadow="never">
            ...
            <el-button type="info" plain style="width:180px" icon="el-icon-download" @click="saveImg" v-show="img_status">保存并绑定</el-button>
            <span style="margin:0px 0px 0px 10px;" v-show="img_status"><font size="3">绑定类型 : </font></span>
            <RadioGroup v-model="saveInfo.type" style="margin:0px 0px 5px 10px;" v-show="img_status" @on-change="selectType">
                <Radio :label="0"><font size="2">整图</font></Radio>
                <Radio :label="1"><font size="2">区域</font></Radio>
            </RadioGroup>
        </el-card>
...
export default class CameraView extends Vue {
    ...
    private openBtnStatus = {type:"info",disabled:false,icon:"el-icon-video-camera",title:"开启摄像头"};
    private takeBtnStatus = {type:"info",disabled:true,icon:"el-icon-camera",title:"拍摄照片"};
    ...
    private openCamera(){
        ...
        promise.then((MediaStream)=> {
            ...
            this.takeBtnStatus = {type:"info",disabled:true,icon:"el-icon-camera",title:"拍摄照片"};
            this.$notify({title: '摄像头异常!',message: '', type: 'error',duration:1500});
		});
    }
    private takePhoto(){
        if(this.img_status){
            this.takeBtnStatus = {type:"info",disabled:false,icon:"el-icon-camera",title:"拍摄照片"};
        ...
        switch(this.saveInfo.type){
            ...
            case 1:
                if(this.saveInfo.cut){
                    ...
                    this.$notify({title: '请选取区域!',message: '', type: 'warning',duration:1500});
                }
                break;
        }
    }
    ...
    private saveOK(){
        this.saveing = true;
        let content = {type:this.saveInfo.type,info:this.saveInfo.type?this.saveInfo.info:{}};
        let bind_info:any = {type:"toDB",route:"binding",job:"add",info:{prjname:this.$store.state.project_info.current_prj,msg:{id:this.s_clid,pid:this.s_module,content:content}}};
    ...
    private disposePhotoData(){
        ...
        let s_req = {
            type : "toSer",
            job : "savePhoto",
            info : {prjname : this.$store.state.project_info.current_prj, type:this.saveInfo.type, info:this.saveInfo.info, id:this.s_clid, img_data : u8arr}
        }
        this.$store.state.app_info.pis.write(s_req);
    }
    private saveCancel(){
