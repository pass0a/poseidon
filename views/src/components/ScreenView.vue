<template>
  <div>
    ...
    <el-card :body-style="{ padding: '8px' }" style="margin:5px 10px 0px 10px;" shadow="never">
      ...
      <el-button style="margin:5px 0px 0px 10px;" plain icon="el-icon-loading" v-show="btnStatus">
        <strong>同步中...</strong>
      </el-button>
      <span style="margin:0px 0px 0px 10px;">
        <strong>
          <font size="3">同步模式 : {{ readSyncMode }}</font>
        </strong>
      </span>
      <el-divider direction="vertical"></el-divider>
      <span style="margin:0px 0px 0px 10px;">
        <strong>
          <font size="3">启动模式 : {{ start_mode }}</font>
        </strong>
      </span>
      <el-divider direction="vertical"></el-divider>
      <span style="margin:0px 0px 0px 10px;">
...
export default class ScreenView extends Vue {
  ...
  private showImage() {
    ...
    if (this.$store.state.screen_info.status) {
      img.src =
        "http://127.0.0.1:6003/getImage?image=" +
        this.$store.state.screen_info.path +
        "&id=" +
        this.idx++;
      this.stopDraw = false;
      this.sync_count++;
    ...
    if (this.s_clid && this.s_clid.length) {
      ...
      let bind_info: any = {
        ...
        info: {
          ...
          msg: { id: this.s_clid, pid: this.s_module, content: this.info }
        }
      };
      switch (this.bind_type) {
        case 0:
