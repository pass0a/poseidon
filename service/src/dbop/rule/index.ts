...
function checkVersion(data: any, pis: any, RuleModel: any) {
	RuleModel.findOne({ id: 'action' }, (err: any, list: any) => {
		if (!err) {
			...
			if (u_data.length) {
				RuleModel.updateOne({ id: 'action' }, { $push: { content: { $each: u_data } } }, function(
					err: any,
					mg: any
				) {
					if (!err) {
...
function copyPrj(data: any, pis: any, RuleModel: any) {
	RuleModel.aggregate([ { $out: data.info.msg.name + '_rule' } ], function(err: any) {
		if (!err) {
			data.info = true;
			pis.write(data);
		}
