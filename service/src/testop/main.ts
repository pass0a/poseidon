...
async function startTest(){
    if (await createdLink()) {
		readConfig();
		let list:any = await notifyToLinkMgr({type:ToDB,route:"status",job:"getTestCases",info:{prjname:prjname}});
...
async function runTest() {
	...
	outputFile(stopflag);
	let results = await notifyToLinkMgr({type : ToDB, route:"results", job:"getRet",info:{prjname:prjname},data:{ok:0,ng:0}}); // 获取测试数据
	...
	if(get_screen){
		...
		if(!match_ret.ret) result = 5;
			...
			if(match_ret.obj.valid || match_num >= caseInfo.config.test_info.match){
				let msg:any = {x : match_ret.obj.x, y : match_ret.obj.y, click_type:cmd.click_type, cfg : caseInfo.config};
...
async function clickPoi(cmd: any, caseData: any) {
	let poi_info:any = await notifyToLinkMgr({type:ToDB,route:"binding",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
...
async function slide(cmd: any, caseData: any) {
	let slide_info:any = await notifyToLinkMgr({type:ToDB,route:"binding",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
...
async function button(cmd: any, caseData: any) {
	let buttonCmd:any = await notifyToLinkMgr({type:ToDB,route:"buttons",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
	for (let i = 0; i < buttonCmd.content.length; i++) {
		if(i==0&&caseInfo.config.da_server.others_flag){
			await notifyToLinkMgr({type:'toCom',job:'sendData',info:{name:"da_arm",cmd:"others",msg:caseInfo.config.da_server}});
		}
		let ct = buttonCmd.content[i];
		for (let j = 0; j < ct.length; j++) {
			// 暂支持串口发送
			let info = { event: buttonCmd.event, ct: ct[j] };
			await notifyToLinkMgr({type:'toCom',job:'sendData',info:{name:"da_arm",cmd:cmd.action,msg:info}});
		}
...
async function pcan(cmd: any, caseData: any){
	let pcanCmd:any = await notifyToLinkMgr({type:ToDB,route:"pcan",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
...
async function adbCmd(cmd: any, caseData: any){
	let adbInfo = await notifyToLinkMgr({type:ToDB,route:"adb",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
...
async function qgBox(cmd: any, caseData: any) {
	...
	let data:any = await notifyToLinkMgr({type:'toQgBox',info:{module:cmd.module, cfg:caseInfo.config.qg_box}});
	let result:any,note:any;
	if (!data.ret) {
		result = cmd.b_type == "0" ? (parseFloat(data.data) > cmd.b_volt ? 0 : 1):(parseFloat(data.data) == cmd.b_volt ? 0 : 1);
...
async function group(cmd: any, caseData: any) {
	let groupContent:any = await notifyToLinkMgr({type:ToDB,route:"group",job:"getDoc",info:{prjname:prjname,id:cmd.id}});
	let groupStepLoopList:any = [],result:any; 
	for (let j = 0; j < groupContent.length; j++) { // 0: 成功 1: 失败 2: 连接错误 3: 组合步骤进行暂停 4: 摄像头异常 5:无图片
		...
		for (let i = 0; i < stepLoop; i++) {
			...
			if (act_ret.ret) {
				groupStepLoopList.push({g_idx:j,g_step_loop_idx:i,g_step_loop_ret:act_ret.ret,g_step_loop_info:act_ret.data});
			}else{
...
async function readyForTest() {
	...
	if (progress_info.total) {
		...
		let ret = uartsSet.size ? await notifyToLinkMgr({type:'toCom',job:'openCom',info:Array.from(uartsSet),cfg:caseInfo.config}) : 1;
...
function readConfig() {
	...
	if(caseInfo.config.test_info==undefined)caseInfo.config.test_info = {error_exit:0,error_music:1,match:90};
	if(caseInfo.config.pcan_info==undefined)caseInfo.config.pcan = {baudrate:"baud_100k",hardware_type:"ISA_82C200",io_port:"0100",interrupt:"3"};
}
